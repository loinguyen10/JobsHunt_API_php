...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['uid']) && isset($data['gender']) 
        ...
        && isset($data['minSalary']) && isset($data['maxSalary']) && isset($data['currency'])) {
            ...
            $sql = "INSERT INTO jh_user_profile_recommend_setting(uid, gender, job, educationId, yearExperience, workProvince, minSalary, maxSalary, currency, userID) VALUES ('$uid', '$gender', '$job', '$educationId', '$yearExperience', '$workProvince', '$minSalary', '$maxSalary', '$currency','$uid')";
            

            if ($conn->query($sql) === TRUE) {
                $response['success'] = 1;
                $response['message'] = 'create setting successfully';
            } else {
                header("HTTP/1.1 500 Internal Server Error");
                $response['success'] = 0;
                $response['message'] = 'create setting unsuccessfully + ' . $conn->error;
            }
